...
VERSION=$2
AWS_S3_BUCKET=$3
RELEASE_DIRS="releases/botwaf"
BOTWAF_REPO="wl4g/botwaf"
...
function upload_artifacts() {
  ...
  # ├── v0.1.0
  # │   ├── botwaf-darwin-amd64-pyo3-v0.1.0.sha256sum
  # │   └── botwaf-darwin-amd64-pyo3-v0.1.0.tar.gz
  # └── v0.2.0
  #    ├── botwaf-darwin-amd64-pyo3-v0.2.0.sha256sum
  #    └── botwaf-darwin-amd64-pyo3-v0.2.0.tar.gz
...
function download_artifacts_from_github() {
  if [ "$DOWNLOAD_ARTIFACTS_FROM_GITHUB" == "true" ]; then
    ...
    RELEASES_API_RESPONSE=$(curl -s -H "Accept: application/vnd.github.v3+json" "https://api.github.com/repos/$BOTWAF_REPO/releases/latest")

    # Extract download URLs for the artifacts.
    # Exclude source code archives which are typically named as 'botwaf-<version>.zip' or 'botwaf-<version>.tar.gz'.
    ASSET_URLS=$(echo "$RELEASES_API_RESPONSE" | jq -r '.assets[] | select(.name | test("botwaf-.*\\.(zip|tar\\.gz)$") | not) | .browser_download_url')
