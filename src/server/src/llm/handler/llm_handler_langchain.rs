...
impl LangchainLLMHandler {
    pub async fn new() -> Self {
        ...
        let call_opts = CallOptions::new()
            .with_max_tokens(config::get_config().botwaf.llm.generate.max_tokens)
            .with_temperature(config::get_config().botwaf.llm.generate.temperature)
            .with_candidate_count(config::get_config().botwaf.llm.generate.candidate_count)
            // TODO: whether the support configuration of this items?
            // .with_functions(Vec::new())
            // .with_stop_words(Vec::new())
            .with_top_k(config::get_config().botwaf.llm.generate.top_k)
            .with_top_p(config::get_config().botwaf.llm.generate.top_p)
            // .with_seed(0)
            .with_function_call_behavior(FunctionCallBehavior::Auto);
        let openai_llm = OpenAI::new(call_openai_config)
            .with_model(config::get_config().botwaf.llm.generate.model.to_owned())
